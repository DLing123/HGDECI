...
def find_neighbor_node(s_id, ifprint=False):
    try:
...
def find_neighbors_event_concept_pair(source: str, target: str, ifprint=False):
    ...
    if s not in cpnet_simple.nodes() or t not in cpnet_simple.nodes():
        ...
        return None, None
    snf_res = find_neighbor_node(s, ifprint)
    tnf_res = find_neighbor_node(t, ifprint)
...
def sample_negs(event_info, tag):
    global cpnet, cpnet_simple
    if cpnet is None or cpnet_simple is None:
        load_cpnet('../../data/cpnet/cpnet.en.pruned.graph')

    sample_neighs = random.sample(event_info[tag + '_nbs']['neighbors'],
                                  min(30, len(event_info[tag + '_nbs']['neighbors'])))
    # add negative samples
    temp = concept2id[event_info[tag]['concept']]
    if len(sample_neighs) < 30:
        neg_temp = []
        for i in range(temp+1, temp+5000):
            if i not in sample_neighs and i in cpnet_simple.nodes():
                neg_temp.append(i)
        sample_neighs += random.sample(neg_temp, 30-len(sample_neighs))

    negative_nbs = []
    for nid in sample_neighs:
        if len(set(cpnet_simple[nid])) < 3:
            continue
        nf_res = find_neighbor_node(nid)

        nf_res_tuple = []
        for i, j in zip(nf_res["neighbors"], nf_res["rel"]):
            if i != concept2id[event_info[tag]['concept']]:
                nf_res_tuple.append((i, j[0]))

        random_nbs = random.sample(nf_res_tuple, min(4, len(nf_res_tuple)))
        temp = {'nb_id': nid, 'neighbors': [], 'rel': []}
        for i in random_nbs:
            temp['neighbors'].append(i[0])
            temp['rel'].append(i[1])
        negative_nbs.append(temp)
        if len(negative_nbs) >= 5:
            break
    return negative_nbs
...
def sort_neighbors(event_info, score_info, tag, threshold):
                        ...
                        for cid in rel2nid[rel]:
                            if len(cp_ids) >= 6:
                                ...
                                if cid[0] not in cp_ids and cid[1] > 0.45:
                                    cp_ids.append(cid[0])
                                    rel_ids.append(rel)
            assert len(cp_ids) == len(rel_ids)

        pos_temp = [(i, j) for i, j in zip(cp_ids, rel_ids)]
        if len(pos_temp) > 3:
            pos_neighbor = random.sample(pos_temp, len(pos_temp) - 2)
        elif 3 >= len(pos_temp) >= 2:
            pos_neighbor = random.sample(pos_temp, len(pos_temp) - 1)
        else:
            pos_neighbor = random.sample(pos_temp, len(pos_temp))
        pos_ids, pos_rel = [], []
        for i, j in pos_neighbor:
            pos_ids.append(i)
            pos_rel.append(j)

        return cp_ids, rel_ids, pos_ids, pos_rel
    ...
    if event_info[tag + '_nbs'] is not None:
        ...
        neighbors, rel, pos_neigh, pos_rel = limited_knows(rel2nid, num)
        p_neighbors = {'neighbors': pos_neigh, 'rel': pos_rel}
        n_neighbors = sample_negs(event_info, tag)
        return {'neighbors': neighbors, 'rel': rel, 'p_neighbors': p_neighbors, 'n_neighbors': n_neighbors}
    else:
        return None
...
if __name__ == "__main__":
    # find_neighbors('../../data/story/ground_res_c.jsonl', '../../data/cpnet/concept.txt',
    #                '../../data/cpnet/cpnet.en.pruned.graph',
    #                '../../data/story/neighbors.raw.jsonl')
    # score_neighbors('../../data/story/neighbors.raw.jsonl', '../../data/transe/glove.transe.sgd.ent.npy',
    #                 '../../data/transe/glove.transe.sgd.rel.npy', '../../data/cpnet/concept.txt',
    #                 '../../data/story/neighbors.scores.jsonl')
    prune_neighbors('../../data/story/neighbors.raw.jsonl', '../../data/story/neighbors.scores.jsonl',
                    '../../data/story/neighbors.pruned1.limited_6.jsonl', 1)
