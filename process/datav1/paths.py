...
def find_paths_event_concept_pair(source: str, target: str, ifprint=False):
    ...
    try:
        ...
        s_nbs, t_nbs = list(set(cpnet_simple[s])), list(set(cpnet_simple[t]))
        s_nbs = random.sample(s_nbs, min(5, len(s_nbs)))
        t_nbs = random.sample(t_nbs, min(5, len(t_nbs)))
        s_t_nbs = [(s_i, t_j) for s_i, t_j in zip(s_nbs, t_nbs)]
        for s_i in s_nbs:
            for t_j in t_nbs:
                if len(s_t_nbs) < 10:
                    if (s_i, t_j) not in s_t_nbs:
                        s_t_nbs.append((s_i, t_j))
                else:
                    break
            if len(s_t_nbs) >= 10:
                break
        for s_i, t_j in s_t_nbs:
            temp = []
            for p in nx.shortest_simple_paths(cpnet_simple, source=s_i, target=t_j):
                if len(temp) < 7:
                    if len(p) > 8:
                        break
                    if len(p) > 4 and p not in all_path:
                        temp.append(p)
                else:
                    break
            temp = random.sample(temp, min(3, len(temp)))
            if len(temp) < 3:
                continue
            else:
                nega_path.append(temp)
    ...
    for item in nega_path:
        ...
        for p in item:
            ...
            for src in range(len(p) - 1):
                ...
                tgt_concept = p[src + 1]
                rel_list = get_edge(src_concept, tgt_concept)
                rl.append(rel_list[0])
            temp.append({"path": p, "rel": rl})
        n_pf_res.append(temp)

    return pf_res, n_pf_res
        ...
        for p in sen['event_pair']:
            if p[0]['concept'] is not None and p[1]['concept'] is not None:
                if p[0]['concept'] == p[1]['concept']:
                    pfr.append({'e1': p[0], 'e2': p[1], 'paths': None, 'n_paths': None})
                else:
                    pf_res, n_pf_res = find_paths_event_concept_pair(p[0]['concept'], p[1]['concept'])
                    if len(pf_res) == 0:
                        pf_res, n_pf_res = None, None
                    pfr.append({'e1': p[0], 'e2': p[1], 'paths': pf_res, 'n_paths': n_pf_res})  # [{}, {}, {}] a sen
            else:
                pfr.append({'e1': p[0], 'e2': p[1], 'paths': None, 'n_paths': None})
...
if __name__ == "__main__":
    # find_paths('../../data/story/ground_res_c.jsonl', '../../data/cpnet/concept.txt',
    #            '../../data/cpnet/cpnet.en.pruned.graph', '../../data/story/paths.raw.jsonl')
    # score_paths('../../data/story/paths.raw.jsonl', '../../data/transe/glove.transe.sgd.ent.npy',
    #             '../../data/transe/glove.transe.sgd.rel.npy', '../../data/cpnet/concept.txt',
    #             '../../data/story/paths.scores.jsonl')
    prune_paths('../../data/story/paths.raw.jsonl', '../../data/story/paths.scores.jsonl',
                    '../../data/story/paths.pruned.limited_6.jsonl', 6)
    # find_paths_event_concept_pair('crime', 'arrest', True)
